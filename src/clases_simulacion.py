...
class Team:
    ...
    def compute_metrics(self, df_metrics: pd.DataFrame):
        ...
        if "fifa_ranking" in df_metrics.columns:
            self.fifa_ranking = df_metrics["fifa_ranking"].item()
            self.squad_value = df_metrics["squad_value"].item()
            self.avg_age = df_metrics["avg_age"].item()
            self.squad_size = df_metrics["squad_size"].item()
...
class Knockouts:
    ...
    def predict_xg_matches(self, round):  
        ...
        for m in self.fixtures:
            f1 = m.s1
            f2 = m.s2
            is_home_1 = f1.name in ["United States", "Canada", "Mexico"]
            is_home_2 = f2.name in ["United States", "Canada", "Mexico"]
            log_sv1 = np.log10(f1.squad_value + 1) if f1.squad_value > 0 else np.nan
            log_sv2 = np.log10(f2.squad_value + 1) if f2.squad_value > 0 else np.nan
            data_rows.append({
                'date': self.game_id, 'team': f1.name, 'opponent': f2.name,
                'elo': f1.elo, 'opponent_elo': f2.elo, 'is_home': is_home_1, 'tournament_num': 5,
                'confed': f1.confed, 'rival_confed': f2.confed,
                'gf_prom_5': f1.gf_prom_5, 'gc_prom_5': f1.gc_prom_5, 'elo_prom_5': f1.elo_prom_5,
                'gf_prom_15': f1.gf_prom_15, 'gc_prom_15': f1.gc_prom_15, 'PCA_1': f1.pca_1, 'PCA_2': f1.pca_2,
                'rival_gf_prom_5': f2.gf_prom_5, 'rival_gc_prom_5': f2.gc_prom_5, 'rival_elo_prom_5': f2.elo_prom_5,
                'rival_gf_prom_15': f2.gf_prom_15, 'rival_gc_prom_15': f2.gc_prom_15, 'rival_PCA_1': f2.pca_1, 'rival_PCA_2': f2.pca_2,
                'fifa_ranking': f1.fifa_ranking, 'log_squad_value': log_sv1, 'avg_age': f1.avg_age,
                'rival_fifa_ranking': f2.fifa_ranking, 'rival_log_squad_value': log_sv2, 'rival_avg_age': f2.avg_age
            })
            
            data_rows.append({
                'date': self.game_id, 'team': f2.name, 'opponent': f1.name,
                'elo': f2.elo, 'opponent_elo': f1.elo, 'is_home': is_home_2, 'tournament_num': 5,
                'confed': f2.confed, 'rival_confed': f1.confed,
                'gf_prom_5': f2.gf_prom_5, 'gc_prom_5': f2.gc_prom_5, 'elo_prom_5': f2.elo_prom_5,
                'gf_prom_15': f2.gf_prom_15, 'gc_prom_15': f2.gc_prom_15, 'PCA_1': f2.pca_1, 'PCA_2': f2.pca_2,
                'rival_gf_prom_5': f1.gf_prom_5, 'rival_gc_prom_5': f1.gc_prom_5, 'rival_elo_prom_5': f1.elo_prom_5,
                'rival_gf_prom_15': f1.gf_prom_15, 'rival_gc_prom_15': f1.gc_prom_15, 'rival_PCA_1': f1.pca_1, 'rival_PCA_2': f1.pca_2,
                'fifa_ranking': f2.fifa_ranking, 'log_squad_value': log_sv2, 'avg_age': f2.avg_age,
                'rival_fifa_ranking': f1.fifa_ranking, 'rival_log_squad_value': log_sv1, 'rival_avg_age': f1.avg_age
            })
