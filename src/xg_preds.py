...
from data_augmentation import load_squad_data, add_squad_features, get_extra_features
...
def train_model(name):
    ...
    extra_features = get_extra_features()
    ...
    # --- Features de Plantilla (FIFA ranking, valor squad, edad) ---
    if squad is not None:
        print("3. Añadiendo features de plantilla (FIFA ranking, valor squad)...")
        df_historia = add_squad_features(df_historia, squad)
        if len(df_mundial) > 0:
            df_mundial = add_squad_features(df_mundial, squad)

    # =====================================================================
    # 4. ENTRENAMIENTO DE XGBOOST
    # =====================================================================
    print("4. Entrenando el motor XGBoost...")
    features = [
    ...
    # =====================================================================
    print("5. Generando predicciones para el Mundial 2026...")

    # Sacamos la FOTO FIJA (El último partido de cada equipo antes del Mundial)
    df_historia.sort_values(by="date", inplace=True)
    foto_fija_cols = ['gf_prom_5', 'gc_prom_5', 'elo_prom_5', 'gf_prom_15', 'gc_prom_15', 'PCA_1', 'PCA_2', 'confed']
    if squad is not None:
        foto_fija_cols += ['fifa_ranking', 'squad_value', 'avg_age', 'squad_size']
    foto_fija_equipos = df_historia.groupby("team")[foto_fija_cols].last().reset_index()

    foto_fija_rivales = foto_fija_equipos.copy()
    rival_col_names = ['opponent'] + [f'rival_{c}' for c in foto_fija_cols]
    foto_fija_rivales.columns = rival_col_names
    ...
    for nombre_jornada, df_jornada in jornadas.items():
        ...
        if squad is not None:
            df_jornada["log_squad_value"] = df_jornada["squad_value"].apply(lambda v: np.log10(v + 1) if pd.notna(v) else np.nan)
            df_jornada["rival_log_squad_value"] = df_jornada["rival_squad_value"].apply(lambda v: np.log10(v + 1) if pd.notna(v) else np.nan)
